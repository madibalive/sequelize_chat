...
module.exports = (sequelize, DataTypes) => {
  ...
  const notify_via_subscription = async function (instance, options) {
    let t;
    try {
      ...
      conversation.setLastMessage(instance);
      conversation.increment('messageCount');
      await conversation.save();
  ...
  const markChatAsRead = async function (instance, options) {
    let t;
    try {
      let { user_id, conversation_id } = instance;
      let subscription = await sequelize.models.Subscription({
        where: {
          user_id,
          conversation_id
        },
        include: [{ model: sequelize.models.Conversation, as: 'Conversation' }]
      });

      if (!subscription) return;
      t = await sequelize.transaction();

      subscription.lastReadMessageId = subscription.Conversation.last_message_id;
      subscription.lastSeenCount = subscription.Conversation.messageCount;
      await subscription.save({ t });
      await t.commit();
    } catch (error) {
      await t.rollback();
  ...
  Message.afterCreate(notify_via_subscription);
