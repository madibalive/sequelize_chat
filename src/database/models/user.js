...
module.exports = (sequelize, DataTypes) => {
  ...
  const createNonExistingConversation = async function (context) {
    try {
      let conversation = await sequelize.model.Conversation.findExistingChat(context.user_id, context.otherOtherIds, t);
      if(!conversation){
        conversation = await sequelize.model.Conversation.createChat(context.myUserId,context.otherOtherIds,context.groupName)
      }
      return conversation;
    } catch (error) {
    }
