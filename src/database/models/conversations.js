...
module.exports = (sequelize, DataTypes) => {
  ...
  const addUserToChat = async function (user_id, conversation_id) {
    let conversation = await sequelize.models.Conversation.findByPk(conversation_id);
    let user = await conversation.setUser(user_id);
    return user;
  };

  const subscribeToChat = async function (userIds, conversation_id) {
    let t;
    try {
  ...
  const deleteConversations = async function (userid, conversationIds) {
    ...
    try {
      ...
      // set conversation as deleted
      let subs = await sequelize.models.Subscription.update(
        {
          deleted: true
        },
        {
  ...
  const findExistingChat = async function (user_id, otherUserIds) {
    let conversation;
    // failed for groups search
  ...
  const getConversation = async function (conversationId) {
    let conversation = await sequelize.models.Conversation.findByPk(conversationId, {
      include: [
        {
          model: sequelize.models.Message,
          as: 'LastMessage'
  ...
  Conversation.getConversation = getConversation;
  Conversation.addUserToChat = addUserToChat;
  Conversation.subscribeToChat = subscribeToChat;
